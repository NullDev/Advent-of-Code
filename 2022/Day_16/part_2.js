...
const s = (op, r, cur, rem, el, p = vals.find(f => f.n === cur) || { costs: {}, r: 0 }) => {
    if (rem <= 0) return;
    ((result = Math.max(r, result)) || 1) && !op.includes(cur) // @ts-ignore
        ? (s([...op, cur], r + p.r * rem, cur, rem - 1, el) || 1) && (!el && s([...op, cur], r + p.r * rem, "AA", 25, true))
        : Object.keys(p.costs)
            .filter(f => !op.includes(f))
            .forEach(k => s(op, r, k, rem - p.costs[k], el));
};
...
console.log("HIGHEST PRESSURE (26M): " + result);
